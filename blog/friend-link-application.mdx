...
桌面表单按 `name | url`、`avatar | backgroundImage`、整行 `bio`、`rss | 一键申请` 四行排列。每个字段都是左侧标签、右侧固定宽度输入区，长内容只在输入框内部滚动，不会撑开组件。`bio`、`avatar` 和 `backgroundImage` 的默认 placeholder 分别为“可选，站点描述或个人 bio。”、“可选，圆形头像地址。”和“可选，卡片背景图地址。”。
