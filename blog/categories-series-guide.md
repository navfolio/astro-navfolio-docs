...
如果希望某些博客文章在归档、分类页或标签页中优先展示，可以在文章 frontmatter 中加入 `sticky` 字段：

```yaml
sticky: true
```

当多篇文章都需要置顶时，也可以使用数字控制优先级。数字越大，排序越靠前：

```yaml
sticky: 10
```

`sticky: true` 会按默认优先级 `1` 处理；`sticky: false`、`sticky: 0`、负数或缺少该字段时，文章都会保持普通排序。普通文章仍然按发布日期从新到旧排列。

置顶只影响 `/blog/`、分页归档、分类页和标签页这些浏览列表。系列页仍然按照发布时间从早到晚形成阅读顺序，RSS 和首页最近活动也继续保持时间线逻辑。
